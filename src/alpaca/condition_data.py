...
def condition_data(dataframe):
    ...
    for ticker in dataframe.index:
        stop_loss = dataframe['Closing Price'][ticker] - \
            dataframe['Closing Price'][ticker] * .0515
        stop_loss_limit = dataframe['Closing Price'][ticker] - \
            dataframe['Closing Price'][ticker] * .06
        take_profit_limit = dataframe['Closing Price'][ticker] + \
            dataframe['Closing Price'][ticker] * .02
        buy_list.append(dict(
            symbol=str(dataframe['Stock'][ticker]),
            take_profile_limit=str(round(take_profit_limit, 3)),
            stop_loss_price=str(round(stop_loss, 3)),
            stop_loss_price_limit=str(round(stop_loss_limit, 3)),
            price=str(dataframe['Closing Price'][ticker])
        ))
        print('buy_list data: ', buy_list)
        # alpaca_list['stock'].append(ticker)
